...
function measure() {
  const out = {};
  // **All ten authored chapters since M29**, walked in ladder order. Three
  // bodies were authored and seven flew a generated survey chapter, which could
  // not be regressed because it is regenerated per seed; now that every body is
  // content, every body gets a recorded outcome.
  const authored = PLANET_ORDER
    .map((pid) => Object.values(CHAPTERS).find((c) => c.planet === pid))
    .filter(Boolean)
    .flatMap((c) => c.levels);
  for (const lvl of [...LEVELS, ...authored]) {
